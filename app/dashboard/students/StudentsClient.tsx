...
export default function StudentsClient({ 
  ...
  // 2. Apply search & set queries
  const filteredStudents = useMemo(() => {
    return enrolledStudents.filter((std) => {
      const sec = getStudentSection(std).toUpperCase();
      if (selectedSetFilter !== "All") {
        if (selectedSetFilter === "A" && !sec.includes("A")) return false;
        if (selectedSetFilter === "B" && !sec.includes("B")) return false;
      }
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full md:w-auto">
          ...
          <div className="w-full sm:w-36">
            <select
              ...
              onChange={(e) => setSelectedSetFilter(e.target.value as any)}
              className="w-full bg-gray-50 border border-gray-200 rounded-xl px-3 py-2 text-sm text-gray-800 font-bold focus:outline-none focus:ring-2 focus:ring-blue-500 cursor-pointer"
            >
